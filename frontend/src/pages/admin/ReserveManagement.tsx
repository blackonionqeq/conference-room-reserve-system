...
export function ReserveManagement() {
	...
	async function changeStatus(id: number, status: '1'|'2'|'3') {
		const url = status === '1' ? 
		'confirm' : status === '2' ?
		'reject' : status === '3' ?
		'unbind' : 'confirm'
		await api.patch(`/reservation/${url}/${id}`)
	...
	const columns: ColumnsType<ReservationItem> = useMemo(() => [
		...
		{ title: '预订人', dataIndex: 'room', render: (_, record) => record.user.username },
		...
		{ title: '操作', render: (_, record) => <div>
			{
				record.status !== '0' ? '' : <div>
					<Popconfirm title='确认' description='确认通过申请？' okText='确认' cancelText='取消' onConfirm={() => changeStatus(record.id, '1')}>
						<a href="#">通过</a>
					</Popconfirm>
					<br />
					<Popconfirm title='确认' description='确认驳回申请？' okText='确认' cancelText='取消' onConfirm={() => changeStatus(record.id, '2')}>
						<a href="#">驳回</a>
					</Popconfirm>
					<br />
					<Popconfirm title='确认' description='确认解除申请？' okText='确认' cancelText='取消' onConfirm={() => changeStatus(record.id, '3')}>
						<a href="#">解除</a>
					</Popconfirm>
				</div>
			}
	...
	const searchConferenceRooms = useCallback(async ({
		...
		const {data} = await api.get<ReservationsReturn>('/reservation/admin/list', {
			params: {
				username,
				roomName,
				roomLocation,
